function onOpen() {
  ...
  ui.createMenu('Render')
      ...
      .addItem('Image by Search', 'maingsearch')
      .addSeparator()
...
function maingsearch() {
  ...
  var value = SpreadsheetApp.getUi().prompt('Enter any keyword: ').getResponseText().split(/ +/g).join('+')
  var s = JSON.parse(UrlFetchApp.fetch('https://pixelsinimage.herokuapp.com/apigsearch?q='+value).getContentText())['url']
  var resp = JSON.parse(UrlFetchApp.fetch('https://pixelsinimage.herokuapp.com/api?u='+s))
